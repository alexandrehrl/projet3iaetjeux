...
Le ratio des temps $t_{\text{spe}}/t_{\text{etu}} \approx 2{,}6\times$ à $n = 2\,000$ s'explique directement par le ratio d'itérations $\approx 4{,}8\times$ : chaque itération côté parcours est certes en $O(1)$ (plus rapide que $O(\log \mathrm{cap})$), mais le nombre total de propositions est bien supérieur.
...
\begin{itemize}
  ...
  \item Côté parcours : $\approx 5{,}3n$ propositions en moyenne (phénomène analogue au problème du coupon collector : chaque parcours doit parcourir plus d'étudiants pour remplir son quota face aux refus).
\end{itemize}

À $n = 2\,000$ : côté etu $\approx 11\%$ du pire cas ($n \times m = 20\,000$), côté parcours $\approx 53\%$ -- tous deux restent bien en-deçà du pire cas théorique.
...
On observe une croissance \textbf{linéaire} en $n$ pour les deux variantes, conformément à la théorie. À $n = 2\,000$, \GS{} côté parcours est $\approx 2{,}6\times$ plus lent ($3{,}04$ ms vs $1{,}15$ ms) malgré une complexité asymptotique inférieure -- l'overhead pratique vient du nombre de propositions $\approx 5\times$ supérieur.
...
On observe également une croissance \textbf{linéaire} en $n$, bien en-deçà du pire cas $n \times m$. La version étudiants est nettement plus économique ($\approx 1{,}1n$ vs $5{,}3n$ propositions) car la max-heap évite les aller-retours superflus entre étudiants libres et parcours pleins.
